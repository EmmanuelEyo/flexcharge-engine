...
export default function WalletDrawer({
  ...
  const [activeTab, setActiveTab] = useState<
    "transactions" | "topup" | "deduct" | "settings"
  >("transactions");
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-50 flex flex-col animate-in slide-in-from-right-8 duration-300 border-l border-slate-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
            ...
            <div className="space-y-3">
              {loading ? (
                ...
                transactions.map((tx) => (
                  <div
                    key={tx._id}
                    className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-indigo-100 transition-colors"
                  >
                    ...
                    <div className="text-right">
                      <p
                        className={`text-sm font-bold ${tx.type === "credit" ? "text-emerald-600" : "text-slate-900"}`}
                      >
                        {tx.type === "credit" ? "+" : "-"}
          ...
          {(activeTab === "topup" || activeTab === "deduct") && (
            <form
              onSubmit={(e) =>
                handleAction(activeTab === "topup" ? "top-up" : "deduct", e)
              }
